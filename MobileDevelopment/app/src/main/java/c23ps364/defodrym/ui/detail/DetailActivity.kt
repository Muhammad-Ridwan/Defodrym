...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val defodrymResId = resources.getIdentifier("defodrym", "string", packageName)
        val baseUrl = if (defodrymResId != 0) getString(defodrymResId) else ""
        val dfd = label.replace(" ", "%20")
        val url = "$baseUrl$dfd"

        GlobalScope.launch(Dispatchers.IO) {
            val result = fetchFoodDetails(url)
            runOnUiThread {
                processFoodDetails(result)
            }
        }
    }

    private fun fetchFoodDetails(url: String): String? {
    ...
    private fun processFoodDetails(result: String?) {
        if (result != null) {
            try {
